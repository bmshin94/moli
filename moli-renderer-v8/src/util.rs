...
pub(super) fn call_global_bridge_method<'s>(
    ...
    crate::script_execution::run(scope, |scope| method.call(scope, bridge.into(), args))
}
...
pub(crate) fn call_script_visible_function<'s>(
    ...
    crate::script_execution::run(scope, |scope| function.call(scope, receiver, args))
}
...
pub(crate) fn utf16_previous_scalar_boundary(units: &[u16], offset: usize) -> usize {
    ...
    if previous > 0
        && (0xDC00..=0xDFFF).contains(&units[previous])
        && (0xD800..=0xDBFF).contains(&units[previous - 1])
    {
        previous -= 1;
    }
    previous
...
pub(crate) fn utf16_next_scalar_boundary(units: &[u16], offset: usize) -> usize {
    let offset = offset.min(units.len());
    if offset == units.len() {
        return offset;
    }
    ...
    if next < units.len()
        ...
        && (0xDC00..=0xDFFF).contains(&units[next])
    {
        next += 1;
    }
    next
