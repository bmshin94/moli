...
async fn start_standalone_document_write_page_for_page_id(
    ...
    while pending
        ...
        .is_none()
    {
        // Native preload progress can park phase one before it reaches the
        // document.write script. Execute those real Page turns first.
        let (outcome, owner_wake_rx) = resume_standalone_document_write_page(pending).await;
        let ParseTimePageVmCreationOutcome::PendingPhaseOne(
            PendingPhaseOneResidence::ClosedInputPageWork { runtime, started },
        ) = outcome
        else {
            panic!("the held document.write script must keep phase one pending");
        };
