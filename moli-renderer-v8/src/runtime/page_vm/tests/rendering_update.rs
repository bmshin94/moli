...
#[tokio::test(flavor = "current_thread")]
async fn screenshot_preserves_slot_inheritance_across_reassignment_and_fallback() {
    run_page_vm_async_test(async move {
        let loader = crate::network::ResourceRequestClient::new(&FetchConfig::default())?;
        let mut page = test_page_vm_with_loader_and_document_url(
            &loader,
            Vec::new(),
            Url::parse("https://example.com/shadow-style-inheritance.html")?,
        );
        page.vm_mut()
            .set_layout_policy(moli_page_types::LayoutPolicy::OnDemand);
        let fixture = include_str!("../../../../tests/fixtures/shadow-style-inheritance.html");
        ...
        for (phase, style_check_count) in [7, 7, 6, 5, 7].into_iter().enumerate() {
            page.vm_mut().eval(&format!("setSlotStylePhase({phase})"))?;
            for geometry in [false, true] {
                if geometry {
                    page.vm_mut()
                        .screenshot_layout_snapshot(moli_layout::PaintViewport::new(800, 600, 1.0))?
                        .expect("slot inheritance fixture must retain a layout root");
                }
                let checks: serde_json::Value = serde_json::from_str(&page.vm_mut().eval(
                    &format!("JSON.stringify(collectSlotStyleChecks({geometry}))"),
                )?)?;
                let checks = checks.as_array().expect("slot inheritance checks");
                // Every phase retains all eight boxes, including hidden fallback.
                assert_eq!(
                    checks.len(),
                    style_check_count + if geometry { 8 } else { 0 }
                );
                for check in checks
                    .iter()
                ...
                {
                    failures.push(
                        serde_json::json!({"phase": phase, "geometry": geometry, "check": check}),
                    );
                }
            }
        }
    ...
    .await
    .expect("slot inheritance should update in style and at the layout barrier");
}
...
#[tokio::test(flavor = "current_thread")]
async fn screenshot_positions_logical_block_flow_before_geometry_and_paint() {
    run_page_vm_async_test(async move {
        let loader = crate::network::ResourceRequestClient::new(&FetchConfig::default())?;
        let mut page = test_page_vm_with_loader_and_document_url(
            &loader,
            Vec::new(),
            Url::parse("https://example.com/logical-block-flow.html")?,
        );
        page.set_viewport_surface(Some(crate::protocol_types::ViewportSurface {
            ...
            screen_avail_height: 1840,
        }))?;
        page.vm_mut()
            .set_layout_policy(moli_page_types::LayoutPolicy::OnDemand);
        let fixture = include_str!("../../../../tests/fixtures/logical-block-flow.html");
        page.vm_mut().eval(&format!(
            "document.open();document.write({});document.close()",
            serde_json::to_string(fixture)?
        ))?;
        ...
        let checks: serde_json::Value = serde_json::from_str(
            &page
                .vm_mut()
                .eval("JSON.stringify(collectLogicalBlockChecks())")?,
        )?;
        let checks = checks.as_array().expect("logical block checks");
    ...
    .await
    .expect("logical block geometry and paint fixture should run");
}
